...
There are three cups on a table, at positions A, B, and C. At the start, there is a ball hidden under the cup at
position B.
...
def cup_swapping(swaps):
    ...
    for swap in swaps:
        if ball_pos in swap:
            ball_pos = ''.join(cup for cup in swap if cup != ball_pos)
    return ball_pos


if __name__ == '__main__':
    assert cup_swapping(["AB", "CA"]) == "C"

    assert cup_swapping(["AC", "CA", "CA", "AC"]) == "B"

    assert cup_swapping(["BA", "AC", "CA", "BC"]) == "A"

...
Create a function that takes in a list of prices for a customer's shopping cart and returns the prices with the discount
 applied. Round all prices to the cent.
Examples
...
Notes
    ...
    The deal applies to sets of three products: if a customer gets 9 products, she will get the three cheapest ones for
    free, but if she gets 10 or 11 products, she will still get three for free. Buying a 12th product would get her a
    fourth free product.
...
def discount(lst):
    ...
    price_original = sum(lst)
    price_discounted = sum(sorted(lst)[len(lst) // 3:])
    discount_pct = price_discounted / price_original
    return [round(discount_pct * p, 2) for p in lst]


if __name__ == '__main__':
    assert discount([2.99, 5.75, 3.35, 4.99]) == [2.47, 4.74, 2.76, 4.12]
    # First product for free.
    #
    ...
    assert discount([5.75, 14.99, 36.83, 12.15, 25.30, 5.75, 5.75, 5.75]) == [5.16, 13.45, 33.06, 10.91, 22.71, 5.16,
                                                                              5.16, 5.16]
    # First and sixth products for free (see second note).

...
def advanced_sort(lst):
    el_counts = {el: lst.count(el) for el in lst}
    return [[el for _ in range(elcount)]
            for el, elcount in el_counts.items()]


if __name__ == '__main__':
    assert advanced_sort([2, 1, 2, 1]) == [[2, 2], [1, 1]]

    assert advanced_sort([5, 4, 5, 5, 4, 3]) == [[5, 5, 5], [4, 4], [3]]

    assert advanced_sort(["b", "a", "b", "a", "c"]) == [["b", "b"], ["a", "a"], ["c"]]

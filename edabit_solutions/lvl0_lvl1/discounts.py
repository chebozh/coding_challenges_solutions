...
The discount is the percentage of the original price (i.e the discount of "75%" to 12 would be 9 as opposed to
taking off 75% (making 3)).
...
def get_discounts(nums, d):
    discount = int(d.replace('%', '')) / 100
    return [n * discount for n in nums]


if __name__ == '__main__':
    assert get_discounts([2, 4, 6, 11], "50%") == [1, 2, 3, 5.5]
    assert get_discounts([10, 20, 40, 80], "75%") == [7.5, 15, 30, 60]
    assert get_discounts([100], "45%") == [45]

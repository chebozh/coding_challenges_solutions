...
def is_narcissistic(n):
    n_str = str(n)
    n_digits = len(str(n_str))
    return sum(int(x)**n_digits for x in n_str) == n


if __name__ == '__main__':
    assert is_narcissistic(8208) is True
    assert is_narcissistic(9) is True
    assert is_narcissistic(22) is False

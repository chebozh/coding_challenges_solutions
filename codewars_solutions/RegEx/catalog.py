...
def catalog(s, article):
    prod_prx_qty = re.findall(r'<name>([a-zA-z\s]*)</name><prx>(\d*[.,]?\d*)</prx><qty>(\d*)', s)
    result_string = ''
    found_prods = []

    for tup in prod_prx_qty:
        if article in tup[0]:
            found_prods.append('{} > prx: ${} qty: {}'.format(tup[0], tup[1], tup[2]))

    if len(found_prods) == 0:
        return 'Nothing'
    elif len(found_prods) > 1:
        for prod in found_prods:
            result_string += prod + '\r\n'

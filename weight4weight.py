...
def order_weight(strng):
    ...
    preliminary_list = [(n, sum(int(x) for x in n), split_string.index(n)) for n in
                        split_string]
    secondary_list = sorted(preliminary_list, key=lambda x: (x[1], x[0]))
    result = [n[0] for n in secondary_list]
    return " ".join(result)
